...
# Imposta localizzazione italiana per formattare valute, date, ecc.
try:
    locale.setlocale(locale.LC_ALL, 'it_IT.UTF-8')
except locale.Error:
    try:
        locale.setlocale(locale.LC_ALL, 'Italian_Italy.1252') # Comune su Windows
    except locale.Error:
        st.warning("Localizzazione italiana ('it_IT.UTF-8' o 'Italian_Italy.1252') non disponibile sul sistema. La formattazione di valute e numeri potrebbe usare lo standard inglese.")
...
try:
    ...
    # --- MODIFICA: Calcolo Errore Sovrastima Punti ---
    df_cleaned['Errore_Sovrastima_PT'] = np.nan # Inizializza a NaN
    # Calcola solo se entrambe le colonne Media Punti Stimati e Punteggio Finale esistono e non sono NaN
    ...
    if df_filtered_final.empty:
        ...
        avg_overestimation_error = np.nan # Sovrastima Media PT (quando presente)
        ...
        # Calcolo Sovrastima Media PT (quando presente)
        ...
        col2.metric(f"{colonna_stake} Totale (W/L)", f"{locale.currency(total_stake_wl, grouping=True) if pd.notna(total_stake_wl) else 'N/D'}")
        col3.metric(f"P/L Totale (W/L)", f"{locale.currency(total_pl_wl, grouping=True) if pd.notna(total_pl_wl) else 'N/D'}")
        ...
        col7.metric(f"{colonna_stake} Medio (W/L)", f"{locale.currency(avg_stake_wl, grouping=True) if pd.notna(avg_stake_wl) else 'N/A'}")
        ...
        with st.container(border=True):
             ...
             r_col1.metric("Max Drawdown Storico (€)", f"{locale.currency(max_drawdown_global, grouping=True) if pd.notna(max_drawdown_global) else 'N/A'}", help="Massima perdita storica dal picco precedente (calcolata su tutti i dati).")
             ...
             r_col3.metric("VaR Storico 95% (Giornaliero)", f"{locale.currency(var_95_daily, grouping=True) if pd.notna(var_95_daily) else 'N/D'}", help="Perdita giornaliera massima attesa nel 5% dei casi peggiori (basata sui dati filtrati).")
        st.markdown("---")
        ...
        if not df_results_filtered_final.empty:
            ...
            if not daily_summary_chart.empty:
                ...
                fig_daily.update_yaxes(title_text="P/L Netto Giornaliero (€)", tickformat="€,.2f", secondary_y=False, showgrid=False); fig_daily.update_yaxes(title_text="Numero Scommesse", secondary_y=True, showgrid=False); fig_daily.update_xaxes(showgrid=False)
        ...
        if not df_results_filtered_final.empty and 'Cumulative P/L Filtered' in df_results_filtered_final.columns and df_results_filtered_final['Cumulative P/L Filtered'].notna().any():
            ...
            fig_cum_pl.update_yaxes(title="P/L Cumulativo (€)", showgrid=False, tickformat="€,.2f"); fig_cum_pl.update_xaxes(title="Data", showgrid=False)
        ...
        # --- MODIFICA: Grafico 6 - Errore Sovrastima vs Probabilità ---
        ...
        if colonna_prob in df_results_filtered_final.columns and 'Errore_Sovrastima_PT' in df_results_filtered_final.columns:
            # Filtra per dati W/L con Probabilità e Errore Sovrastima validi
            df_plot_prob_error = df_results_filtered_final[
                df_results_filtered_final[colonna_prob].notna() &
                df_results_filtered_final['Errore_Sovrastima_PT'].notna() # Usa la nuova colonna
            ].copy()

            if not df_plot_prob_error.empty:
                 # Aggiungi colonne formattate per tooltip
                df_plot_prob_error['Prob Formattata'] = df_plot_prob_error[colonna_prob].map(lambda x: f"{x:.1%}" if pd.notna(x) else 'N/A')
                # Formatta la nuova colonna errore per il tooltip
                ...
                # Crea scatter plot usando la nuova colonna errore sull'asse Y
                fig_prob_error = px.scatter(df_plot_prob_error,
                                            x=colonna_prob,
                                            y='Errore_Sovrastima_PT', # Asse Y modificato
                                            color='Esito_Standard',
                                            title=f"Errore Sovrastima Punti vs. {colonna_prob} (Periodo Filtrato)",
                                            # Aggiorna etichette assi
                                            labels={
                                                colonna_prob: 'Probabilità Stimata',
                                                'Errore_Sovrastima_PT': 'Errore Sovrastima Punti', # Etichetta Y modificata
                                            ...
                                            hover_data={
                                                ...
                                                'Errore Sovrastima Formattato': True, # Tooltip modificato
                                                colonna_prob: False,
                                                'Errore_Sovrastima_PT': False # Nasconde valore grezzo Y
                                            },
                ...
                st.info(f"Nessun dato valido (con Prob. Stimata ed Errore Sovrastima calcolabile) per questo grafico.")
            st.markdown("---")
        else:
            st.info(f"Le colonne '{colonna_prob}' o '{colonna_media_pt_stimati}'/'{colonna_ris_finale}' (per calcolare l'errore) non sono disponibili.")
            st.markdown("---")
        # --- FINE MODIFICA GRAFICO 6 ---
        ...
        with col_c:
             if colonna_prob in df_results_filtered_final.columns:
                 ...
                 if not df_plot_prob_pl.empty:
                     df_plot_prob_pl['Stake Formattato'] = df_plot_prob_pl[colonna_stake].map(lambda x: locale.currency(x, grouping=True) if pd.notna(x) else 'N/A'); df_plot_prob_pl['P/L Formattato'] = df_plot_prob_pl['P/L'].map(lambda x: locale.currency(x, grouping=True) if pd.notna(x) else 'N/A'); df_plot_prob_pl['Prob Formattata'] = df_plot_prob_pl[colonna_prob].map(lambda x: f"{x:.1%}" if pd.notna(x) else 'N/A')
                     ...
                     fig_prob_pl.update_traces(marker=dict(line=dict(width=0))); fig_prob_pl.update_layout(template='plotly_white'); fig_prob_pl.update_xaxes(title="Probabilità Stimata", showgrid=False, tickformat=".1%"); fig_prob_pl.update_yaxes(title="P/L (€)", showgrid=False)
        ...
        with col_d:
              if colonna_stake in df_filtered_final.columns:
                 ...
                 if not df_plot_stake.empty:
                     ...
                     fig_stake_hist.update_traces(marker_line_width=0); fig_stake_hist.update_layout(template='plotly_white', bargap=0.1); fig_stake_hist.update_yaxes(title="Numero Scommesse", showgrid=False); fig_stake_hist.update_xaxes(title="Importo Puntato (€)", showgrid=False)
        ...
        cols_to_format_percent = [colonna_prob, colonna_edge]; cols_to_format_currency = [colonna_stake, 'P/L']
        # --- MODIFICA: Includi nuova colonna errore nella formattazione punti ---
        ...
        for col in cols_to_format_currency:
             if col in df_display_filtered.columns: df_display_filtered[col] = pd.to_numeric(df_display_filtered[col], errors='coerce').map(lambda x: locale.currency(x, grouping=True) if pd.notna(x) else "")
        ...
        # --- MODIFICA: Aggiorna ordine colonne per tabella ---
        colonne_display_ordine = [
            ...
            'Errore_Sovrastima_PT', # Inserisci nuova colonna errore
            'Esito_Standard', 'P/L'
        ]
        # --- FINE MODIFICA ---
        ...
        # Usa df_cleaned per il download (dati puliti ma non filtrati dalla UI, con nuova colonna errore)
...
# --- Gestione Errori Critici ---
    ...
    # --- MODIFICA: Correzione SyntaxError ---
    st.error(f"ERRORE CRITICO: Colonna fondamentale non trovata: {e}.")
    # Sposta l'if su una nuova riga indentata
    if 'df' in locals():
        st.info(f"Colonne lette: {df.columns.tolist()}")
    st.stop()
    # --- FINE MODIFICA ---
except Exception as e:
    st.error(f"ERRORE CRITICO IMPREVISTO: {e}")
    st.exception(e)
    st.stop()
